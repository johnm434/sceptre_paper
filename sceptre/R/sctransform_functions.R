...
regularize_thetas <- function(genes_log_gmean, theta, theta_regularization = "log_theta", bw_adjust = 3, plot_me = FALSE) {
  if (bw_adjust == 0) {
    ...
    if (plot_me) {
      df1 <- tibble(mean_gene_exp = c(dispersion_par, fitted_dispersion_par), log_theta = c(genes_log_gmean_step1, x_points), regularized = c(rep(FALSE, length(genes_log_gmean_step1)), rep(TRUE, length(x_points))))
